 ...
 * 
 * pwrutil governor for the LITTLE CLUSTER.
 *
...
struct lcgov_policy {
	...
	struct lcgov_tunables *tunables;
...
static DEFINE_PER_CPU(struct lcgov_cpu, lcgov_cpu);
static DEFINE_PER_CPU(struct lcgov_tunables, cached_tunables);

/************************ Governor internals ***********************/

static bool lcgov_should_update_freq(struct lcgov_policy *sg_policy, u64 time)
{
	s64 delta_ns;
...
static bool lcgov_up_down_rate_limit(struct lcgov_policy *sg_policy, u64 time,
				     unsigned int next_freq)
{
	s64 delta_ns;
...
static void lcgov_update_commit(struct lcgov_policy *sg_policy, u64 time,
				unsigned int next_freq)
{
	struct cpufreq_policy *policy = sg_policy->policy;

	if (lcgov_up_down_rate_limit(sg_policy, time, next_freq))
		return;
 ...
 * @sg_cpu: pwrutil cpu object to compute the new frequency for.
 ...
 */
static unsigned int get_next_freq(struct lcgov_cpu *sg_cpu, unsigned long util,
				  unsigned long max)
{
	struct lcgov_policy *sg_policy = sg_cpu->sg_policy;
	struct cpufreq_policy *policy = sg_policy->policy;
	struct lcgov_tunables *tunables = sg_policy->tunables;
	unsigned int freq;
...
static void lcgov_get_util(unsigned long *util, unsigned long *max, u64 time)
{
...
static void lcgov_set_iowait_boost(struct lcgov_cpu *sg_cpu, u64 time,
				   unsigned int flags)
{
...
static void lcgov_iowait_boost(struct lcgov_cpu *sg_cpu, unsigned long *util,
			       unsigned long *max)
{
...
static void lcgov_update_single(struct update_util_data *hook, u64 time,
				unsigned int flags)
{
	struct lcgov_cpu *sg_cpu = container_of(hook, struct lcgov_cpu, update_util);
	struct lcgov_policy *sg_policy = sg_cpu->sg_policy;
	struct cpufreq_policy *policy = sg_policy->policy;
	unsigned long util, max;
	unsigned int next_f;

	lcgov_set_iowait_boost(sg_cpu, time, flags);
	sg_cpu->last_update = time;

	if (!lcgov_should_update_freq(sg_policy, time))
		return;
	...
	if (flags & SCHED_CPUFREQ_DL) {
		...
		lcgov_get_util(&util, &max, time);
		lcgov_iowait_boost(sg_cpu, &util, &max);
		util = cap_clamp_cpu_util(smp_processor_id(), util);
		next_f = get_next_freq(sg_cpu, util, max);
	}
	lcgov_update_commit(sg_policy, time, next_f);
}

static unsigned int lcgov_next_freq_shared(struct lcgov_cpu *sg_cpu,
					   ...
					   unsigned int flags)
{
	...
	for_each_cpu(j, policy->cpus) {
		...
		lcgov_iowait_boost(j_sg_cpu, &util, &max);
...
static void lcgov_update_shared(struct update_util_data *hook, u64 time,
				unsigned int flags)
{
	struct lcgov_cpu *sg_cpu = container_of(hook, struct lcgov_cpu, update_util);
	struct lcgov_policy *sg_policy = sg_cpu->sg_policy;
	...
	if (lcgov_should_update_freq(sg_policy, time)) {
		next_f = lcgov_next_freq_shared(sg_cpu, util, max, flags);
		lcgov_update_commit(sg_policy, time, next_f);
	}
...
static void lcgov_work(struct kthread_work *work)
{
	struct lcgov_policy *sg_policy = container_of(work, struct lcgov_policy, work);
...
static void lcgov_irq_work(struct irq_work *irq_work)
{
	struct lcgov_policy *sg_policy;

	sg_policy = container_of(irq_work, struct lcgov_policy, irq_work);

	/*
	 * For Real Time and Deadline tasks, pwrutil governor shoots the
	 * frequency to maximum. And special care must be taken to ensure that
	 * this kthread doesn't result in that.
	 *
	 * This is (mostly) guaranteed by the work_in_progress flag. The flag is
	 * updated only at the end of the lcgov_work() and before that pwrutil
...
static struct lcgov_tunables *global_tunables;
static DEFINE_MUTEX(global_tunables_lock);

static inline struct lcgov_tunables *to_lcgov_tunables(struct gov_attr_set *attr_set)
{
	return container_of(attr_set, struct lcgov_tunables, attr_set);
}

static DEFINE_MUTEX(min_rate_lock);

static void update_min_rate_limit_us(struct lcgov_policy *sg_policy)
{
...
{
	struct lcgov_tunables *tunables = to_lcgov_tunables(attr_set);
...
static struct attribute *lcgov_attributes[] = {
...
static struct kobj_type lcgov_tunables_ktype = {
	.default_attrs = lcgov_attributes,
	.sysfs_ops = &governor_sysfs_ops,
};

/********************** cpufreq governor interface *********************/
#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_PWRUTIL_LC
static
#endif
struct cpufreq_governor cpufreq_gov_pwrutil_lc;

static struct lcgov_policy *lcgov_policy_alloc(struct cpufreq_policy *policy)
{
	struct lcgov_policy *sg_policy;
	...
	init_irq_work(&sg_policy->irq_work, lcgov_irq_work);
	mutex_init(&sg_policy->work_lock);
	raw_spin_lock_init(&sg_policy->update_lock);
	return sg_policy;
}

static void lcgov_policy_free(struct lcgov_policy *sg_policy)
{
	mutex_destroy(&sg_policy->work_lock);
	kfree(sg_policy);
}

static int lcgov_kthread_create(struct lcgov_policy *sg_policy)
{
	struct task_struct *thread;
	...
	init_kthread_work(&sg_policy->work, lcgov_work);
	init_kthread_worker(&sg_policy->worker);
	thread = kthread_create(kthread_worker_fn, &sg_policy->worker,
				"lcgov:%d",
				cpumask_first(policy->related_cpus));
	if (IS_ERR(thread)) {
		pr_err("failed to create lcgov thread: %ld\n", PTR_ERR(thread));
...
static void lcgov_kthread_stop(struct lcgov_policy *sg_policy)
{
	/* kthread only required for slow path */
...
static struct lcgov_tunables *lcgov_tunables_alloc(struct lcgov_policy *sg_policy)
{
	struct lcgov_tunables *tunables;
...
static void lcgov_tunables_free(struct lcgov_tunables *tunables)
{
	if (!have_governor_per_policy())
...
static void store_tunables_data(struct lcgov_tunables *tunables,
		struct cpufreq_policy *policy)
{
	struct lcgov_tunables *ptunables;
...
static void get_tunables_data(struct lcgov_tunables *tunables,
		struct cpufreq_policy *policy)
{
	struct lcgov_tunables *ptunables;
	unsigned int lat;
...
static int lcgov_init(struct cpufreq_policy *policy)
{
	struct lcgov_policy *sg_policy;
	struct lcgov_tunables *tunables;
	int ret = 0;
	...
	sg_policy = lcgov_policy_alloc(policy);
	if (!sg_policy)
		return -ENOMEM;

	ret = lcgov_kthread_create(sg_policy);
	if (ret)
		goto free_sg_policy;
	...
	tunables = lcgov_tunables_alloc(sg_policy);
	if (!tunables) {
		ret = -ENOMEM;
		goto stop_kthread;
	}
	...
	ret = kobject_init_and_add(&tunables->attr_set.kobj, &lcgov_tunables_ktype,
				   get_governor_parent_kobj(policy), "%s",
				   cpufreq_gov_pwrutil_lc.name);
	if (ret)
		goto fail;
 ...
 fail:
	...
	lcgov_tunables_free(tunables);

stop_kthread:
	lcgov_kthread_stop(sg_policy);

free_sg_policy:
	mutex_unlock(&global_tunables_lock);

	lcgov_policy_free(sg_policy);
	pr_err("initialization failed (error %d)\n", ret);
	return ret;
}

static int lcgov_exit(struct cpufreq_policy *policy)
{
	struct lcgov_policy *sg_policy = policy->governor_data;
	struct lcgov_tunables *tunables = sg_policy->tunables;
	unsigned int count;
...
static int lcgov_start(struct cpufreq_policy *policy)
{
	struct lcgov_policy *sg_policy = policy->governor_data;
	unsigned int cpu;
	...
	for_each_cpu(cpu, policy->cpus) {
		struct lcgov_cpu *sg_cpu = &per_cpu(lcgov_cpu, cpu);
...
static int lcgov_stop(struct cpufreq_policy *policy)
{
	struct lcgov_policy *sg_policy = policy->governor_data;
	unsigned int cpu;
...
static int lcgov_limits(struct cpufreq_policy *policy)
{
	struct lcgov_policy *sg_policy = policy->governor_data;
...
static int cpufreq_pwrutil_lc_cb(struct cpufreq_policy *policy,
				unsigned int event)
{
	switch(event) {
	case CPUFREQ_GOV_POLICY_INIT:
		return lcgov_init(policy);
	case CPUFREQ_GOV_POLICY_EXIT:
		return lcgov_exit(policy);
	case CPUFREQ_GOV_START:
		return lcgov_start(policy);
	case CPUFREQ_GOV_STOP:
		return lcgov_stop(policy);
	case CPUFREQ_GOV_LIMITS:
		return lcgov_limits(policy);
	default:
		BUG();
	}
}
...
struct cpufreq_governor cpufreq_gov_pwrutil_lc = {
	.name = "pwrutil_lc",
	.governor = cpufreq_pwrutil_lc_cb,
	.owner = THIS_MODULE,
};

static int __init lcgov_register(void)
{
	return cpufreq_register_governor(&cpufreq_gov_pwrutil_lc);
}
fs_initcall(lcgov_register);
