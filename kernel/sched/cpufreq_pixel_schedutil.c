...
#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_PIXEL_SCHEDUTIL
static
#endif
struct cpufreq_governor cpufreq_gov_pixel_schedutil;
...
static int sugov_init(struct cpufreq_policy *policy)
{
	...
	ret = kobject_init_and_add(&tunables->attr_set.kobj, &sugov_tunables_ktype,
				   ...
				   cpufreq_gov_pixel_schedutil.name);
	if (ret)
		goto fail;
...
static int cpufreq_pixel_schedutil_cb(struct cpufreq_policy *policy,
				unsigned int event)
{
	switch(event) {
...
#endif
struct cpufreq_governor cpufreq_gov_pixel_schedutil = {
	.name = "pixel_schedutil",
	.governor = cpufreq_pixel_schedutil_cb,
	.owner = THIS_MODULE,
};

static int __init sugov_register(void)
{
	return cpufreq_register_governor(&cpufreq_gov_pixel_schedutil);
}
