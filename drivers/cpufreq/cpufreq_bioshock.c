...
static inline cputime64_t get_cpu_time(unsigned int cpu, cputime64_t *wall)
{
				      ...
				      const char *buf, size_t count)
{
	...
	for_each_online_cpu(j) {
		...
		dbs_info->prev_cpu_idle = get_cpu_time(j,
...
static void dbs_check_cpu(struct cpu_dbs_info_s *this_dbs_info)
{
	...
	for_each_cpu(j, policy->cpus) {
		...
		cur_idle_time = get_cpu_time(j, &cur_wall_time);
...
static inline void dbs_timer_init(struct cpu_dbs_info_s *dbs_info)
{
	...
	dbs_info->enable = 1;
	INIT_DELAYED_WORK_ONSTACK(&dbs_info->work, do_dbs_timer);
				   ...
				   unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		...
		for_each_cpu(j, policy->cpus) {
			...
			j_dbs_info->prev_cpu_idle = get_cpu_time(j,
