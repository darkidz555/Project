...
u32 cancun_gpu_range_start_freq = 342000000;
u32 cancun_gpu_range_end_freq = 414000000;
u32 cancun_gpu_range_enter_time = 1000000;
u32 cancun_gpu_range_out_time = 500000;
u32 cancun_gpu_opt_gold_freq = 1109400;
u32 cancun_compact_opt_silver_freq = 1670400;
u32 cancun_compact_opt_gold_freq = 1728000;
u32 cancun_compact_disable = 0;
u32 cancun_rel = CPUFREQ_RELATION_H;

/* Cancun DBoT variable */
#define MAX_DIFF_TEMP           900
u32 cancun_dbot_opt_freq = 2035200;
		...
		unsigned int loadadjfreq)
{
	...
	do {
		...
		if(cancun_gov_en){
			if(gpubound_status){
				if(pcpu->policy->cpu >= 4){
					if(freq >= cancun_gpu_opt_gold_freq){
...
void check_cancun_status(int cpu,u64 now)
{
	...
	if(cancun_is_dbot && cancun_is_game && per_cpu_info){
		diff_temp = per_cpu_info[4].temp - (long)cancun_dbot_vts_temp / 10;
				   ...
				   unsigned long busy)
{
	...
	prev_load = mult_frac(ppol->policy->cpuinfo.max_freq * 100,
				busy, jiffies_to_usecs(tunables->timer_rate));
	return prev_load;
}
		...
		const char *buf, size_t count)
{
	...
	for_each_possible_cpu(cpu) {
		...
		if (t && t->use_sched_load)
			t->timer_rate = usecs_to_jiffies(val_round);
	}
...
static ssize_t show_cancun_gpu_opt_gold_freq(
		struct cpufreq_interactive_tunables *tunables, char *buf)
{
	return sprintf(buf, "%u\n", cancun_gpu_opt_gold_freq);
}

static ssize_t store_cancun_gpu_opt_gold_freq(
		struct cpufreq_interactive_tunables *tunables, const char *buf,
		size_t count)
{
	int ret;
		...
		struct cpufreq_interactive_tunables *tunables, char *buf)
{
	...
	extern unsigned int cancun_dbot_quiet_temp;
	struct cpu_pwr_stats *per_cpu_info = get_cpu_pwr_stats();

	if (!per_cpu_info)
		return sprintf(buf, "[cancun] cannot read per_cpu temp\n");

	return sprintf(buf,"cpu0:%ld cpu4:%ld vts:%ld xo:%d quiet:%d heat:%d\n",
		per_cpu_info[0].temp, per_cpu_info[4].temp,
		cancun_dbot_vts_temp, cancun_dbot_xo_temp, cancun_dbot_quiet_temp,
		heating_status);
}
...
static ssize_t show_cancun_rel(
		struct cpufreq_interactive_tunables *tunables, char *buf)
{
	return sprintf(buf, "%d\n", cancun_rel);
}

static ssize_t store_cancun_rel(
		struct cpufreq_interactive_tunables *tunables, const char *buf,
		size_t count)
{
	int ret;
...
show_store_gov_pol_sys(cancun_gpu_opt_gold_freq);
...
show_store_gov_pol_sys(cancun_rel);
#endif /* CONFIG_LGE_PM_CANCUN */
...
gov_sys_pol_attr_rw(cancun_gpu_opt_gold_freq);
...
gov_sys_pol_attr_rw(cancun_rel);
#endif /*CONFIG_LGE_PM_CANCUN*/
...
static struct attribute *interactive_attributes_gov_sys[] = {
	...
	&cancun_gpu_opt_gold_freq_gov_sys.attr,
...
static struct attribute *interactive_attributes_gov_pol[] = {
	...
	&cancun_gpu_opt_gold_freq_gov_pol.attr,
...
void cpufreq_lock(int cpu, unsigned long lock)
{
	struct cpufreq_interactive_policyinfo *ppol = per_cpu(polinfo, cpu);
	spin_lock_irqsave(&ppol->target_freq_lock, lock);
    spin_lock(&ppol->load_lock);
}
void cpufreq_unlock(int cpu, unsigned long lock)
{
	struct cpufreq_interactive_policyinfo *ppol = per_cpu(polinfo, cpu);
	spin_unlock(&ppol->load_lock);
	spin_unlock_irqrestore(&ppol->target_freq_lock, lock);
}
