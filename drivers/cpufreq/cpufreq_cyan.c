/*
 * drivers/cpufreq/cpufreq_cyan.c
 *
 * Copyright (C) 2010 Google, Inc.
 * Copyright (C) 2015 Emre Harbutoğlu
 *
 ...
 *
 * Author: Emre Harbutoğlu (coder44278@gmail.com)
 * Author: Paul Reioux (reioux@gmail.com) Modified for cyan
 */
 ...
 * sync_freq
 */
static unsigned int up_threshold_any_cpu_load = 25;
...
static int cpufreq_governor_cyan(struct cpufreq_policy *policy,
		unsigned int event);

#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_CYAN
static
#endif
struct cpufreq_governor cpufreq_gov_cyan = {
	.name = "cyan",
	.governor = cpufreq_governor_cyan,
...
static int __init cpufreq_cyan_init(void)
{
	unsigned int i;
	struct cpufreq_interactive_cpuinfo *pcpu;
	...
	return cpufreq_register_governor(&cpufreq_gov_cyan);
}

#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_CYAN
fs_initcall(cpufreq_cyan_init);
#else
module_init(cpufreq_cyan_init);
#endif

static void __exit cpufreq_interactive_exit(void)
{
	cpufreq_unregister_governor(&cpufreq_gov_cyan);
...
MODULE_AUTHOR("Emre Harbutoğlu <coder44278@gmail.com>");
MODULE_AUTHOR("Paul Reioux <reioux@gmail.com>");
MODULE_DESCRIPTION("'cpufreq_cyan' - A cpufreq governor for "
