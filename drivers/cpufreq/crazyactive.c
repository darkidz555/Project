/*
 * drivers/cpufreq/cpufreq_crazyactive.c
 *
...
#include <trace/events/cpufreq_crazyactive.h>

#define DEFAULT_HISPEED_FREQ	1958400

static int active_count;

struct cpufreq_crazyactive_cpuinfo {
...
static DEFINE_PER_CPU(struct cpufreq_crazyactive_cpuinfo, cpuinfo);
 ...
 * timer interval.
 */
#define DEFAULT_ABOVE_HISPEED_DELAY (70 * USEC_PER_MSEC)
...
static int cpufreq_governor_crazyactive(struct cpufreq_policy *policy,
		unsigned int event);

#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_CRAZYACTIVE
static
#endif
struct cpufreq_governor cpufreq_gov_crazyactive = {
	.name = "crazyactive",
	.governor = cpufreq_governor_crazyactive,
...
static void cpufreq_crazyactive_timer_resched(
	struct cpufreq_crazyactive_cpuinfo *pcpu)
{
 ...
 * function.
 */
static void cpufreq_crazyactive_timer_start(int cpu, int time_override)
{
	struct cpufreq_crazyactive_cpuinfo *pcpu = &per_cpu(cpuinfo, cpu);
	unsigned long flags;
	unsigned long expires;
	if (time_override)
...
static unsigned int choose_freq(
	struct cpufreq_crazyactive_cpuinfo *pcpu, unsigned int loadadjfreq)
{
...
static void cpufreq_crazyactive_timer(unsigned long data)
{
	u64 now;
	unsigned int delta_time;
	u64 cputime_speedadj;
	...
	struct cpufreq_crazyactive_cpuinfo *picpu;
	...
	if (pcpu->target_freq >= hispeed_freq &&
		...
		trace_cpufreq_crazyactive_notyet(
	...
	if (pcpu->target_freq == new_freq) {
		trace_cpufreq_crazyactive_already(
			data, cpu_load, pcpu->target_freq,
			pcpu->policy->cur, new_freq);
		goto rearm_if_notmax;
	}

	trace_cpufreq_crazyactive_target(data, cpu_load, pcpu->target_freq,
					 pcpu->policy->cur, new_freq);
...
rearm:
	if (!timer_pending(&pcpu->cpu_timer))
		cpufreq_crazyactive_timer_resched(pcpu);
...
static void cpufreq_crazyactive_idle_start(void)
{
	struct cpufreq_crazyactive_cpuinfo *pcpu =
...
static void cpufreq_crazyactive_idle_end(void)
{
	struct cpufreq_crazyactive_cpuinfo *pcpu =
		&per_cpu(cpuinfo, smp_processor_id());
	...
	if (!timer_pending(&pcpu->cpu_timer)) {
		...
		cpufreq_crazyactive_timer(smp_processor_id());
	}

	up_read(&pcpu->enable_sem);
}

static int cpufreq_crazyactive_speedchange_task(void *data)
{
	unsigned int cpu;
	cpumask_t tmp_mask;
	unsigned long flags;
	...
	while (1) {
		...
		for_each_cpu(cpu, &tmp_mask) {
			...
			for_each_cpu(j, pcpu->policy->cpus) {
				struct cpufreq_crazyactive_cpuinfo *pjcpu =
					&per_cpu(cpuinfo, j);
			...
			if (max_freq != pcpu->policy->cur)
				__cpufreq_driver_target(pcpu->policy,
							max_freq,
							CPUFREQ_RELATION_H);
			trace_cpufreq_crazyactive_setspeed(cpu,
						     pcpu->target_freq,
						     pcpu->policy->cur);
...
static void cpufreq_crazyactive_boost(void)
{
	int i;
	int anyboost = 0;
	unsigned long flags;
...
static int cpufreq_crazyactive_notifier(
...
static struct notifier_block cpufreq_notifier_block = {
	.notifier_call = cpufreq_crazyactive_notifier,
};
			   ...
			   const char *buf, size_t count)
{
	...
	if (boost_val) {
		trace_cpufreq_crazyactive_boost("on");
		cpufreq_crazyactive_boost();
	} else {
		trace_cpufreq_crazyactive_unboost("off");
	}
				...
				const char *buf, size_t count)
{
	...
	trace_cpufreq_crazyactive_boost("pulse");
	cpufreq_crazyactive_boost();
	return count;
}
...
static struct attribute *crazyactive_attributes[] = {
...
static struct attribute_group crazyactive_attr_group = {
	.attrs = crazyactive_attributes,
	.name = "crazyactive",
};

static int cpufreq_crazyactive_idle_notifier(struct notifier_block *nb,
					     unsigned long val,
					     void *data)
{
	switch (val) {
	case IDLE_START:
		cpufreq_crazyactive_idle_start();
		break;
	case IDLE_END:
		cpufreq_crazyactive_idle_end();
		break;
	}
...
static struct notifier_block cpufreq_crazyactive_idle_nb = {
	.notifier_call = cpufreq_crazyactive_idle_notifier,
};
		...
		unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		...
		for_each_cpu(j, policy->cpus) {
			...
			cpufreq_crazyactive_timer_start(j, 0);
		...
		rc = sysfs_create_group(cpufreq_global_kobject,
				&crazyactive_attr_group);
		if (rc) {
			mutex_unlock(&gov_lock);
			return rc;
		}

		idle_notifier_register(&cpufreq_crazyactive_idle_nb);
	...
	case CPUFREQ_GOV_STOP:
		...
		cpufreq_unregister_notifier(
			&cpufreq_notifier_block, CPUFREQ_TRANSITION_NOTIFIER);
		idle_notifier_unregister(&cpufreq_crazyactive_idle_nb);
	...
	case CPUFREQ_GOV_LIMITS:
		...
		for_each_cpu(j, policy->cpus) {
			...
			if (policy->min >= pcpu->target_freq) {
				...
				cpufreq_crazyactive_timer_start(j, expire_time);
			}
...
static void cpufreq_crazyactive_nop_timer(unsigned long data)
{
}

static int __init cpufreq_crazyactive_init(void)
{
	unsigned int i;
	struct cpufreq_crazyactive_cpuinfo *pcpu;
	...
	for_each_possible_cpu(i) {
		...
		pcpu->cpu_timer.function = cpufreq_crazyactive_timer;
		pcpu->cpu_timer.data = i;
		init_timer(&pcpu->cpu_slack_timer);
		pcpu->cpu_slack_timer.function = cpufreq_crazyactive_nop_timer;
	...
	mutex_init(&gov_lock);
	speedchange_task =
		kthread_create(cpufreq_crazyactive_speedchange_task, NULL,
			       "cfcrazyactive");
	...
	return cpufreq_register_governor(&cpufreq_gov_crazyactive);
}

#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_CRAZYACTIVE
fs_initcall(cpufreq_crazyactive_init);
#else
module_init(cpufreq_crazyactive_init);
#endif

static void __exit cpufreq_crazyactive_exit(void)
{
	cpufreq_unregister_governor(&cpufreq_gov_crazyactive);
	kthread_stop(speedchange_task);
	put_task_struct(speedchange_task);
}

module_exit(cpufreq_crazyactive_exit);

MODULE_AUTHOR("Mike Chan <mike@android.com>");
MODULE_DESCRIPTION("'cpufreq_crazyactive' - A cpufreq governor for "
