...
#include "lge/lge_error_detect.h"
#endif
...
int mdss_dsi_on(struct mdss_panel_data *pdata)
{
	...
	ctrl_pdata->need_post_panel_on = false;
#endif
...
#if defined(CONFIG_LGE_DISPLAY_AMBIENT_SUPPORTED)
	if (mdss_dsi_is_panel_on_lp(pdata)) {
		pr_debug("%s: panel already on (lp state)\n", __func__);
		goto end;
	}
#endif
...
#if defined(CONFIG_LGE_MIPI_JOAN_ONCELL_QHD_CMD_PANEL)
	ctrl_pdata->need_post_panel_on = true;
#endif
...
#if defined(CONFIG_LGE_DISPLAY_ERROR_DETECT)
	if (ctrl_pdata->is_first_err_mask) {
		lge_dsi_irq_control(ctrl_pdata, true);
		lge_mdss_dsi_panel_err_cmds_send(ctrl_pdata, DSI_HS_MODE);
		ctrl_pdata->is_first_err_mask = false;
	}
#endif
...
#if defined(CONFIG_LGE_DISPLAY_AMBIENT_SUPPORTED)
	lge_mdss_ambient_config_reset(pdata);
#endif
...
#if defined(CONFIG_LGE_MIPI_JOAN_ONCELL_QHD_CMD_PANEL)
	if (ctrl_pdata->need_post_panel_on == false) {
		pr_debug("panel already on\n");
		return 0;
	}
#endif
...
static char drs_ctrl[1] = {0x29};
static struct dsi_cmd_desc drs_cmd = {
	{DTYPE_DCS_WRITE, 1, 0, 0, 1, sizeof(drs_ctrl)},
	drs_ctrl
};
#endif
...
static void mdss_drs_disp_ctrl(struct mdss_dsi_ctrl_pdata *ctrl, int on)
{
	struct mdss_panel_info *pinfo;
#if !defined(CONFIG_LGE_DISPLAY_BIST_MODE)
	...
	lge_mdss_dsi_bist_ctrl(ctrl, !on);
	mutex_unlock(&ctrl->bist_lock);
#else
	if (on)	drs_ctrl[0] = 0x29;
	else	drs_ctrl[0] = 0x28;
...
static void mdss_dsi_drs_switch_panel_cfg(struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	pt = container_of(pdata->current_timing, struct dsi_panel_timing, timing);
...
static bool mdss_dsi_drs_retry_switch_panel_cfg(struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	int retry_cnt = 5;
	char data;
	int ret = 0;
	...
	while (retry_cnt > 0) {
		...
		mdelay(16);
		ret = lge_mdss_dsi_panel_cmd_read(0x3F, 0x01, &data);
		if(ret < 0)
			pr_err("[Display] %s: Panel Cmd Read Error : %d\n", __func__, ret);

		if (ctrl_pdata->req_resolution == data) {
...
static void mdss_dsi_drs_work_sub(struct work_struct *work)
{
	struct mdss_dsi_ctrl_pdata *ctrl_pdata = NULL;
	struct delayed_work *dw = to_delayed_work(work);
	...
	ctrl_pdata = container_of(dw, struct mdss_dsi_ctrl_pdata, drs_work_sub);
	if (!ctrl_pdata) {
		pr_err("%s: invalid ctrl data\n", __func__);
		return;
	}
	...
	pr_debug("[Display] %s: screen on : state:%d, req:%d, ic:%d, result:%s\n",
			...
			data,
			switch_success ? "success":"fail");

	mutex_lock(&ctrl_pdata->drs_lock);
	if (!switch_success) {
		switch_success = mdss_dsi_drs_retry_switch_panel_cfg(ctrl_pdata);
	}
	...
	if (ctrl_pdata->drs_state == DYNAMIC_RESOLUTION_SWITCH_FINISH) {
		...
		mdss_fb_drs_notify(ctrl_pdata->drs_kobj, 1);
		mdss_drs_disp_ctrl(ctrl_pdata, 1);
	} else {
		if (switch_success) {
			mdss_drs_disp_ctrl(ctrl_pdata, 1);
			ctrl_pdata->drs_state = DYNAMIC_RESOLUTION_SWITCH_FINISH;
		} else {
			pr_err("[Display] %s: panel still off??\n", __func__);
			pr_err("[Display] %s: state : %d, cfg : %d\n",
					__func__,
					ctrl_pdata->drs_state,
					ctrl_pdata->update_panel_cfg);
		}
	}

	ctrl_pdata->drs_state = DYNAMIC_RESOLUTION_SWITCH_IDLE;
	mutex_unlock(&ctrl_pdata->drs_lock);
	complete(&ctrl_pdata->drs_work_done);
}

static void mdss_dsi_drs_work(struct work_struct *work)
{
	int rc;
	struct mdss_dsi_ctrl_pdata *ctrl_pdata = NULL;
	struct delayed_work *dw = to_delayed_work(work);
	...
	ctrl_pdata = container_of(dw, struct mdss_dsi_ctrl_pdata, drs_work);
	if (!ctrl_pdata) {
		pr_err("%s: invalid ctrl data\n", __func__);
		return;
	}

	init_completion(&ctrl_pdata->drs_done);
	init_completion(&ctrl_pdata->drs_work_done);
	...
	rc = wait_for_completion_timeout(&ctrl_pdata->drs_done, KOFF_TIMEOUT);
	if (rc <= 0) {
		mdss_fb_drs_notify(ctrl_pdata->drs_kobj, 2);
		pr_err("[Display] %s: GRAM garbage screen?? : (%d)\n",
				__func__, ctrl_pdata->drs_state);
	} else {
		mdss_dsi_drs_switch_panel_cfg(ctrl_pdata);
		update_cfg = true;
	...
	mutex_lock(&ctrl_pdata->drs_lock);
	ctrl_pdata->drs_state = DYNAMIC_RESOLUTION_SWITCH_FINISH;
	ctrl_pdata->update_panel_cfg = update_cfg;
	mutex_unlock(&ctrl_pdata->drs_lock);
	queue_delayed_work(ctrl_pdata->drs_workq, &ctrl_pdata->drs_work_sub,
			msecs_to_jiffies(17));
}
#endif
				  ...
				  int event, void *arg)
{
	...
	case MDSS_EVENT_PANEL_ULP_TO_LP_MODE:
		...
		break;
	case MDSS_EVENT_PANEL_LP_TO_ULP_MODE:
		rc = lge_mdss_ambient_change_lp_mode(pdata, (int)(unsigned long)arg);
		break;
#if defined(CONFIG_LGE_DISPLAY_CHANGE_PARTIAL_AREA_IN_KICKOFF)
	case MDSS_EVENT_PANEL_CHANGE_PARTIAL_AREA:
		rc = lge_mdss_ambient_change_partial_area(pdata, (int)(unsigned long)arg);
		break;
#if defined(CONFIG_LGE_DISPLAY_BIST_MODE)
	case MDSS_EVENT_PANEL_BIST_OFF:
		rc = lge_mdss_ambient_post_change_partial_area(pdata, (int)(unsigned long)arg);
		break;
#if defined(CONFIG_LGE_DISPLAY_DYNAMIC_RESOLUTION_SWITCH)
	case MDSS_EVENT_PANEL_BIST_VERIFY:
		rc = lge_mdss_ambient_bist_verify(pdata, (int)(unsigned long)arg);
		break;
...
#if defined(CONFIG_LGE_DISPLAY_DYNAMIC_RESOLUTION_SWITCH)
	mutex_init(&ctrl_pdata->drs_lock);
	ctrl_pdata->drs_workq = create_workqueue("mdss_dsi_drs");
	if (!ctrl_pdata->drs_workq) {
		pr_warn("[Display] %s: Warning creating workqueue\n", __func__);
		ctrl_pdata->drs_state =	DYNAMIC_RESOLUTION_SWITCH_NONE;
	} else {
		ctrl_pdata->drs_state =	DYNAMIC_RESOLUTION_SWITCH_IDLE;
		INIT_DELAYED_WORK(&ctrl_pdata->drs_work, mdss_dsi_drs_work);
		INIT_DELAYED_WORK(&ctrl_pdata->drs_work_sub, mdss_dsi_drs_work_sub);
	}
#endif
#if defined(CONFIG_LGE_DISPLAY_BIST_MODE)
	mutex_init(&ctrl_pdata->bist_lock);
#endif
...
static int mdss_dsi_remove(struct platform_device *pdev)
{
	mdss_dsi_res_deinit(pdev);
#if defined(CONFIG_LGE_DISPLAY_ERROR_DETECT)
	lge_dsi_err_detect_remove(platform_get_drvdata(pdev));
#endif
	return 0;
}
...
#if defined(CONFIG_LGE_DISPLAY_ERROR_DETECT)
	lge_dsi_err_detect_init(ctrl_pdata);
#endif
