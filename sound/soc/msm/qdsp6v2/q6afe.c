...
#if defined(CONFIG_SND_SOC_TFA9872)
static int afe_tfa_dsp_read_msg_via_mmap(int dev /* not used */, int buf_size, char *buf)
{
	int result = 0;
	int tfa_port_id = AFE_PORT_ID_TERTIARY_MI2S_RX;
	int index = 0;
...
{
	int ret = -EINVAL;
	int tfa_port_id = AFE_PORT_ID_TERTIARY_MI2S_RX; //AFE_PORT_ID_PRIMARY_MI2S_RX --> AFE_PORT_ID_TERTIARY_MI2S_RX;
	int tfa_apr_pkt_size;
#if defined(AFE_TFADSP_STATIC_MEMORY)
	static struct afe_tfa_dsp_send_msg_t *tfa_dsp_send_msg = NULL;
#else
	struct afe_tfa_dsp_send_msg_t *tfa_dsp_send_msg;
#endif
	int index = 0;
...
#else
	/*alloc memory: to get the continuous memory block for the apr packet.
	 to support the variable size of the tfadsp packet
	 and consider the frame size limit of 2048 bytes;
	 this limit causes the compilation error.
	 the tfadsp requires around max 2300 bytes
	 over the 2048 bytes (tuning tool and etc)
	 but the normal case requires around max 700 bytes per packet*/
	tfa_dsp_send_msg =
		(struct afe_tfa_dsp_send_msg_t *)
		kmalloc(tfa_apr_pkt_size, GFP_KERNEL);
#endif
	if (tfa_dsp_send_msg == NULL) {
	...
	tfa_dsp_send_msg->hdr.src_port = 0;
	tfa_dsp_send_msg->hdr.dest_port = 0;
	...
	// 	(int)tfa_dsp_send_msg->payload.buf[1] (int)tfa_dsp_send_msg->payload.buf[2], (int)tfa_dsp_send_msg->payload.buf[3]);
...
static int __init afe_init(void)
{
	...
	/* tfa_ext_register(afe_tfa_dsp_write_reg, afe_tfa_dsp_send_msg, afe_tfa_dsp_read_msg, NULL); */
	/* Block this code. Because build error (incompatible pointer type) */
	tfa_ext_register(afe_tfa_dsp_send_msg, afe_tfa_dsp_read_msg_via_mmap, NULL);
#endif // CONFIG_SND_SOC_TFA9872
...
#if defined(CONFIG_SND_SOC_TFA9872)
#if defined(AFE_TFADSP_STATIC_MEMORY)
	afe_tfa_dsp_send_msg(-1, 0, NULL, 0, 0);
