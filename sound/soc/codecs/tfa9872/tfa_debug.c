...
char *tfa_cont_bit_name(uint16_t num, unsigned short rev)
{
	...
	case 0x80:
	case 0x81:
		name = tfa_bf2name(tfa9890_bit_names, num); /* my tabel 1st */
		if (strcmp(unknown, name)==0)
			name = tfa_bf2name(tfa1_bit_names, num); /* try generic table */
		break;
	case 0x12:
		name = tfa_bf2name(tfa9887_bit_names, num); /* my tabel 1st */
		if (strcmp(unknown, name)==0)
			name = tfa_bf2name(tfa1_bit_names, num);/* try generic table */
		break;
	case 0x13:
...
char *tfa_cont_ds_name(uint16_t num, unsigned short rev)
{
	...
	case 0x80:
	case 0x81:
		name = tfa_bf2name(tfa9890_datasheet_names, num); /* my tabel 1st */
		if (strcmp(unknown, name)==0)
			name = tfa_bf2name(tfa1_datasheet_names, num); /* try generic table */
		break;
	case 0x12:
		name = tfa_bf2name(tfa9887_datasheet_names, num); /* my tabel 1st */
		if (strcmp(unknown, name)==0)
			name = tfa_bf2name(tfa1_datasheet_names, num);/* try generic table */
		break;
	case 0x13:
...
uint16_t tfa_cont_bf_enum(const char *name, unsigned short rev)
{
	...
	case 0x88:
		...
		if (bfnum==0xffff)
			bfnum = tfa_name2bf(tfa2_bit_names, name);/* try long bitname table */
		break;
	case 0x97:
		bfnum =  tfa_name2bf(tfa1_datasheet_names, name);
		if (bfnum==0xffff)
			bfnum = tfa_name2bf(tfa1_bit_names, name);/* try generic table */
		break;
	case 0x96:
		bfnum =  tfa_name2bf(tfa9896_datasheet_names, name);
		if (bfnum==0xffff)
			bfnum = tfa_name2bf(tfa9896_bit_names, name);/* try generic table */
		break;
	case 0x72:
		bfnum =  tfa_name2bf(tfa9872_datasheet_names, name);
		if (bfnum==0xffff)
			bfnum = tfa_name2bf(tfa9872_bit_names, name);/* try long bitname table */
		break;
	case 0x92:
		bfnum =  tfa_name2bf(tfa9891_datasheet_names, name);
		if (bfnum==0xffff)
			bfnum = tfa_name2bf(tfa9891_bit_names, name);/* try long bitname table */
		break;
	case 0x91:
	case 0x80:
	case 0x81:
		bfnum = tfa_name2bf(tfa9890_datasheet_names, name); /* my tabel 1st */
		if (bfnum==0xffff)
			bfnum = tfa_name2bf(tfa1_datasheet_names, name);/* try generic table */
		if (bfnum==0xffff)
			bfnum = tfa_name2bf(tfa1_bit_names, name); /* try 2nd generic table */
		break;
	case 0x12:
		bfnum = tfa_name2bf(tfa9887_datasheet_names, name); /* my tabel 1st */
		if (bfnum==0xffff)
			bfnum = tfa_name2bf(tfa1_datasheet_names, name);/* try generic table */
		if (bfnum==0xffff)
			bfnum = tfa_name2bf(tfa1_bit_names, name);/* try 2nd generic table */
	...
	case 0x13:
		...
		if (bfnum==0xffff)
			bfnum = tfa_name2bf(tfa9912_bit_names, name);/* try long bitname table */
		break;
